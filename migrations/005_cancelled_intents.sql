CREATE TABLE payment_intents_new (
  id TEXT PRIMARY KEY,
  terminal_badge_id TEXT NOT NULL REFERENCES badges(badge_id),
  merchant_address TEXT NOT NULL,
  amount_lamports INTEGER NOT NULL CHECK(amount_lamports > 0),
  memo TEXT NOT NULL DEFAULT '',
  expires_at INTEGER NOT NULL,
  status TEXT NOT NULL CHECK(status IN ('pending','signed','submitted','confirmed','failed','expired','cancelled')),
  customer_badge_id TEXT,
  customer_nonce TEXT,
  signature TEXT,
  signed_payload_hash TEXT,
  created_by TEXT NOT NULL,
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL,
  intent_nonce TEXT,
  protocol_version INTEGER NOT NULL DEFAULT 1
);

INSERT INTO payment_intents_new
  SELECT id, terminal_badge_id, merchant_address, amount_lamports, memo, expires_at, status,
    customer_badge_id, customer_nonce, signature, signed_payload_hash, created_by, created_at,
    updated_at, intent_nonce, protocol_version
  FROM payment_intents;

DROP TABLE payment_intents;
ALTER TABLE payment_intents_new RENAME TO payment_intents;

CREATE UNIQUE INDEX unique_customer_nonce ON payment_intents(customer_badge_id, customer_nonce)
  WHERE customer_badge_id IS NOT NULL AND customer_nonce IS NOT NULL;
CREATE INDEX intent_status_expiry ON payment_intents(status, expires_at);
CREATE UNIQUE INDEX unique_intent_nonce ON payment_intents(intent_nonce) WHERE intent_nonce IS NOT NULL;
